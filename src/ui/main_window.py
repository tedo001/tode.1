...
class MainWindow(tk.Frame):
    ...
    def setup_drag_drop(self):
        """Register the canvas as a DnD drop target (tkinterdnd2 required)."""
        try:
            from tkinterdnd2 import DND_FILES
            self.player.canvas.drop_target_register(DND_FILES)
            self.player.canvas.dnd_bind("<<Drop>>", self._on_drop)
            log.info("Drag-and-drop registered on canvas")
        except Exception as exc:
            log.warning(f"Drag-and-drop unavailable: {exc}")

    def _on_drop(self, event):
        """Handle a file/folder dropped onto the canvas."""
        if self._busy:
            return
