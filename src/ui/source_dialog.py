...
class SourceDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        self._open_btn = tk.Button(btns, text="Open  →",
                  ...
                  bg="#333344", fg="#666677", relief=tk.FLAT,
                  padx=18, pady=8,
                  font=("Consolas", 10, "bold"),
                  cursor="hand2",
                  state=tk.DISABLED)
        ...
        self._selected_path.trace_add("write", self._on_path_changed)
    ...
    def _on_path_changed(self, *_):
        if self._selected_path.get().strip():
            self._open_btn.config(state=tk.NORMAL, bg=ACCENT, fg="white")
        else:
            self._open_btn.config(state=tk.DISABLED, bg="#333344", fg="#666677")
